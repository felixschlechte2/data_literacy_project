...
def process_data_index(parties, offset):
    ...
    pollutant_data = pollutant_data[['Year', 'Air Pollution Level', 'City']]
    ...
    data = data[['Date', 'Air Pollution Level', 'City', 'Others'] + parties]
    print(data.shape)
    ...
    for index, row in data.iterrows():
        city = row['City']
        year = row['Date']
        if pollutant_data[(pollutant_data['City'] == city) & (pollutant_data['Year'] == year + offset)].any()['Air Pollution Level'] and year + offset < 2025:
            election_result = row[parties]
            pollution_before = row['Air Pollution Level']
            pollution_after = pollutant_data[(pollutant_data['City'] == city) & (pollutant_data['Year'] == year + offset)]['Air Pollution Level'].values[0]
