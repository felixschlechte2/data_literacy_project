...
starting_AQ = 100
starting_party = 1
for city in range(10): # simulate cities
    AQ = []
    party = []
    noise = np.random.normal(0, 1)
    for year in range(4): # simulate years
        df['City'].append(city)
        df['Years'].append(year)
        aq = starting_AQ + year * 0.3 + noise + np.random.normal(0, 0.5)
        p = starting_party + year * 0.4 
        party.append(p)
        AQ.append(aq)
        df['AQ'].append(aq)
        df['party'].append(p)
    plt.plot(party, AQ, color='blue', marker='o')
    starting_AQ -= 1
    starting_party += 2

# linear regression: 
m, b = np.polyfit(df['party'], df['AQ'], 1)
y_pred = m * np.array(df['party']) + b
plt.plot(df['party'], y_pred, color='red', label=f"linear regression")

# fixed effects:
df = pd.DataFrame(df)
df = df.set_index(['City', 'Years'])
mod = PanelOLS.from_formula(
    'AQ ~ party + EntityEffects', 
    data=df
)
res = mod.fit()
slope = res.params["party"]
p_value_party = round(res.pvalues["party"], 4)
print(res.summary)
x = np.linspace(0, 15, 100)
y = slope * x + 90   # offset for visibility
plt.plot(x, y, label=f'fixed effects (w/o TimeEffects) p-value: {p_value_party}', color='green')

plt.title('modellized AQ over election results (each with noise)')
